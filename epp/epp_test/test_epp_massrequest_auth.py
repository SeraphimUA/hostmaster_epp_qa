...
def test_epp_auth_whois_domain(domain, elapsetime, session_epp):
...
def test_epp_auth_whois_contact(contact, elapsetime, session_epp):
    whoisdata = {'object_type': 'contact', 'object_name': contact}
    r_whois = requests.get(f"{url}whois.php", params=whoisdata)
    assert r_whois.elapsed.total_seconds() <= elapsetime

@pytest.mark.eppauth
def test_epp_auth_registrators(session_epp):
    request_time_array = []
    for i in range(200):
...
@pytest.mark.eppauth
def test_epp_auth_profile(session_epp):
    request_time_array = []
    for i in range(200):
